...
public class FakeQuartz2Scheduler implements Scheduler {

    private Map<TriggerKey, JobKey> triggers = new HashMap<>();
    private Map<JobKey, JobDetail> jobs = new HashMap<>();

    @Override
    public Date scheduleJob(JobDetail detail, Trigger trigger) {
        jobs.put(detail.getKey(), detail);
        triggers.put(trigger.getKey(), detail.getKey());
        return new Date();
    }
    ...
    public void triggerJob(JobKey jobKey) throws SchedulerException {
        try {
            JobDetail detail = jobs.get(jobKey);
